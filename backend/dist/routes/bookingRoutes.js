...
const bookingReceiptPdf_1 = require("../lib/bookingReceiptPdf");
const router = (0, express_1.Router)();
const upload = (0, multer_1.default)({ storage: multer_1.default.memoryStorage() });
const MESSAGE_MAX_LEN = 5000;
async function assertBookingMessageAccess(userId, role, bookingId) {
    ...
    if (role === "hotel") {
        const { data: profile } = await supabaseClient_1.supabaseAdmin
            ...
            .single();
        const ph = profile;
        if (ph?.hotel_id && hotelId && ph.hotel_id === hotelId)
            return { ok: true };
        return { ok: false, status: 403, error: "Forbidden" };
    }
    return { ok: false, status: 403, error: "Forbidden" };
}
router.post("/bookings", auth_1.authenticate, upload.single("receipt"), async (req, res) => {
    ...
    const { room_id, check_in, check_out, booking_type, hours: hoursRaw, payment_method, hotel_payment_method_id: hotelPaymentMethodIdRaw, } = req.body;
    const hotelPaymentMethodId = typeof hotelPaymentMethodIdRaw === "string" && hotelPaymentMethodIdRaw.trim()
    ...
    if (Array.isArray(hoursRaw)) {
        ...
        try {
            ...
            hours = Array.isArray(parsed) ? parsed : undefined;
        }
        catch {
            hours = undefined;
        }
    ...
    if (isOnlinePayment && hotelHasConfiguredProviders) {
        ...
        const { data: pmRow, error: pmErr } = await supabaseClient_1.supabaseAdmin
            .from("hotel_payment_methods")
            .select("id, hotel_id")
            .eq("id", hotelPaymentMethodId)
            .single();
        if (pmErr || !pmRow || pmRow.hotel_id !== roomHotelId) {
    ...
    if (effectiveType === "nightly") {
        const { data: availability, error: availabilityError } = await supabaseClient_1.supabaseClient
        ...
        const nights = (new Date(check_out).getTime() - new Date(check_in).getTime()) /
            (1000 * 60 * 60 * 24);
        ...
        const { data: slots, error: slotsError } = await supabaseClient_1.supabaseClient
        ...
        for (const s of slots ?? []) {
            ...
            if (!Number.isFinite(hour))
                continue;
            slotMap.set(hour, Boolean(s.available));
        }
    ...
    const checkInValue = effectiveType === "hourly" && hourlyCheckIn
        ? hourlyCheckIn.toISOString()
        : check_in;
    const checkOutValue = effectiveType === "hourly" && hourlyCheckIn
    ...
    if (roomHotelId && finalBooking && typeof finalBooking.id === "string") {
        const { data: guestProf } = await supabaseClient_1.supabaseAdmin
            .from("profiles")
        ...
        try {
            (0, realtime_1.notifyHotelNewBooking)(roomHotelId, finalBooking.id, guestLabel);
        }
        catch (e) {
...
async function signedRoomMediaUrls(media) {
    ...
    for (const item of media) {
        ...
        if (signed?.signedUrl)
            out.push({ type, url: signed.signedUrl });
    }
    return out;
}
function bookingDetailRoomRow(r) {
...
router.get("/bookings", auth_1.authenticate, async (req, res) => {
    ...
    const { data, error } = await db
        .from("bookings")
        .select("*, rooms(hotel_id, name, hotels(id, name, address, images, profile_image, check_in_time, check_out_time, currency))")
        .eq("user_id", userId)
    ...
    const { data: existingReviews } = await supabaseClient_1.supabaseAdmin
        .from("reviews")
        .select("booking_id")
        .in("booking_id", bookingIds);
    const reviewedBookingIds = new Set((existingReviews ?? []).map((r) => String(r.booking_id)));
    const now = new Date();
    const thisMonthStart = new Date(now.getFullYear(), now.getMonth(), 1).toISOString();
    const thisMonthEnd = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999).toISOString();
    const { data: monthReviews } = await supabaseClient_1.supabaseAdmin
        .from("reviews")
        .select("booking_id")
        .eq("user_id", userId)
        .gte("created_at", thisMonthStart)
        .lte("created_at", thisMonthEnd);
    const monthReviewBookingIds = (monthReviews ?? []).map((r) => String(r.booking_id));
    const hotelsReviewedThisMonth = new Set();
    if (monthReviewBookingIds.length > 0) {
        const { data: br } = await supabaseClient_1.supabaseAdmin
            .from("bookings")
            .select("room_id")
        ...
        if (roomIds.length > 0) {
            const { data: roomsData } = await supabaseClient_1.supabaseAdmin
                .from("rooms")
                .select("hotel_id")
                .in("id", roomIds);
            for (const row of roomsData ?? []) {
                ...
                if (hid)
                    hotelsReviewedThisMonth.add(hid);
            }
        }
    }
    const HOTEL_MONTH_MSG = "You can only rate each property once per month. Try again next month.";
    const enriched = out.map((b) => {
        ...
        let hotelId = roomSingle && typeof roomSingle === "object"
            ? String(roomSingle.hotel_id ?? "") || null
            : null;
        if (!hotelId && roomSingle && typeof roomSingle === "object") {
            const h = roomSingle.hotels;
            const hotel = Array.isArray(h) ? h[0] : h;
            if (hotel?.id)
                hotelId = String(hotel.id);
        }
        const eligibleBase = checkOutPast &&
        ...
        const rate_stay_block_reason = !hasReview && eligibleBase && hotelBlocked ? HOTEL_MONTH_MSG : null;
        return {
            ...b,
...
router.get("/bookings/:bookingId/messages", auth_1.authenticate, async (req, res) => {
    ...
    const enriched = (rows ?? []).map((r) => {
        ...
        const senderName = prof?.full_name?.trim() ||
            prof?.email?.trim() ||
...
router.post("/bookings/:bookingId/messages", auth_1.authenticate, async (req, res) => {
    const rawBid = req.params.bookingId;
    const bookingId = Array.isArray(rawBid) ? rawBid[0] : rawBid;
    if (!bookingId) {
        res.status(400).json({ error: "Invalid booking id" });
    ...
    const { data: inserted, error } = await supabaseClient_1.supabaseAdmin
        .from("booking_messages")
        .insert({
        booking_id: bookingId,
        sender_id: userId,
    ...
    const ins = inserted;
    const { data: prof } = await supabaseClient_1.supabaseAdmin
        .from("profiles")
    ...
    const senderName = p?.full_name?.trim() || p?.email?.trim() || (senderRole === "hotel" ? "Hotel" : "Guest");
    const out = {
        id: ins.id,
        booking_id: bookingId,
    ...
    try {
        (0, realtime_1.emitBookingMessage)(bookingId, out);
        const { data: bRow } = await supabaseClient_1.supabaseAdmin
            .from("bookings")
            .select("user_id, rooms!inner(hotel_id)")
            .eq("id", bookingId)
            .single();
        ...
        if (br?.user_id) {
            ...
            if (hid) {
                await (0, realtime_1.notifyNewChatMessage)({
                    bookingId,
                    senderRole,
...
router.get("/bookings/:id/e-receipt", auth_1.authenticate, async (req, res) => {
    ...
    const { data: booking, error } = await supabaseClient_1.supabaseAdmin
        .from("bookings")
        .select(`id, check_in, check_out, booking_type, total_amount, status, payment_status, payment_method, created_at,
    ...
    const { data: profile } = await supabaseClient_1.supabaseAdmin
        ...
        .single();
    const prof = profile;
    const { data: authUser } = await supabaseClient_1.supabaseAdmin.auth.admin.getUserById(userId);
    const email = prof?.email ?? authUser?.user?.email ?? null;
    const { checkIn, checkOut, issuedAtLabel } = (0, bookingReceiptPdf_1.formatReceiptLabels)(b.check_in, b.check_out, new Date().toISOString());
    const bookingTypeLabel = String(b.booking_type).toLowerCase() === "hourly" ? "Hourly (micro-stay)" : "Nightly";
    (0, bookingReceiptPdf_1.pipeBookingEreceiptPdf)(res, {
        ...
        checkIn,
        checkOut,
        totalAmountLabel: (0, bookingReceiptPdf_1.formatReceiptAmount)(b.total_amount, currency),
        paymentMethod: (0, bookingReceiptPdf_1.paymentMethodLabel)(b.payment_method),
...
router.get("/bookings/:id", auth_1.authenticate, async (req, res) => {
    ...
    const userId = req.user.sub;
    const { data: booking, error: bookingError } = await supabaseClient_1.supabaseAdmin
        .from("bookings")
        .select(`*, rooms(
    ...
    const roomRow = bookingDetailRoomRow(booking.rooms);
    const hotelId = roomRow?.hotel_id ?? undefined;
    let hotel = null;
    if (hotelId) {
        const { data: hotelRow } = await supabaseClient_1.supabaseAdmin
            .from("hotels")
    ...
    let canReview = false;
    let reviewBlockReason = null;
    const { data: reviewRow } = await supabaseClient_1.supabaseAdmin
        .from("reviews")
    ...
    if (reviewRow) {
        ...
        const thisMonthStart = new Date(now.getFullYear(), now.getMonth(), 1).toISOString();
        const thisMonthEnd = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999).toISOString();
        const { data: sameMonthReviews } = await supabaseClient_1.supabaseAdmin
            .from("reviews")
            .select("booking_id")
            .eq("user_id", userId)
            .gte("created_at", thisMonthStart)
            .lte("created_at", thisMonthEnd);
        const monthBids = (sameMonthReviews ?? []).map((r) => String(r.booking_id));
        let sameHotelThisMonth = false;
        if (monthBids.length > 0) {
            const { data: bkRows } = await supabaseClient_1.supabaseAdmin
                .from("bookings")
                .select("room_id")
                .in("id", monthBids);
            const roomIds = [
            ...
            if (roomIds.length > 0) {
                const { data: roomsData } = await supabaseClient_1.supabaseAdmin
                    ...
                    .select("hotel_id")
                    .in("id", roomIds);
                sameHotelThisMonth = (roomsData ?? []).some((row) => row.hotel_id === hotelId);
            }
        }
...
router.post("/bookings/:id/payment-receipt", auth_1.authenticate, upload.single("receipt"), async (req, res) => {
    if (!req.supabaseAccessToken) {
        res.status(401).json({ error: "Supabase session required (send x-supabase-access-token)" });
        return;
    }
    ...
    try {
        ...
        const { data: roomRow } = await supabaseClient_1.supabaseAdmin
            .from("rooms")
            .select("hotel_id")
            .eq("id", roomId)
            .single();
        const hid = roomRow?.hotel_id;
        if (hid) {
            (0, realtime_1.notifyHotelReceiptUploaded)(hid, bookingId);
        }
    }
    catch (e) {
...
router.post("/bookings/:id/review", auth_1.authenticate, async (req, res) => {
    const rawId = req.params.id;
    const bookingId = Array.isArray(rawId) ? rawId[0] : rawId;
    if (!bookingId) {
        res.status(400).json({ error: "Invalid booking id" });
    ...
    const { data: roomInfo } = await supabaseClient_1.supabaseAdmin
        .from("rooms")
        .select("hotel_id")
        .eq("id", booking.room_id)
        .single();
    const targetHotelId = roomInfo?.hotel_id;
    if (!targetHotelId) {
    ...
    const { data: sameMonthReviews } = await supabaseClient_1.supabaseAdmin
        ...
        .gte("created_at", thisMonthStart)
        .lte("created_at", thisMonthEnd);
    const priorBookingIds = (sameMonthReviews ?? []).map((r) => String(r.booking_id));
    let sameHotelThisMonth = false;
    if (priorBookingIds.length > 0) {
        const { data: bookingsForReviews } = await supabaseClient_1.supabaseAdmin
            .from("bookings")
            .select("room_id")
            .in("id", priorBookingIds);
        const roomIds = [
            ...new Set((bookingsForReviews ?? [])
                .map((x) => x.room_id)
                .filter((id) => typeof id === "string" && id.length > 0)),
        ];
        if (roomIds.length > 0) {
            const { data: roomsData } = await supabaseClient_1.supabaseAdmin
                ...
                .select("hotel_id")
                .in("id", roomIds);
            sameHotelThisMonth = (roomsData ?? []).some((row) => row.hotel_id === targetHotelId);
        }
    }
    if (sameHotelThisMonth) {
        res.status(400).json({
    ...
    const { data: review, error: insertError } = await supabaseClient_1.supabaseAdmin
        .from("reviews")
        .insert({
        booking_id: bookingId,
        user_id: userId,
...
router.patch("/bookings/:id/cancel", auth_1.authenticate, async (req, res) => {
