...
router.get("/admin/hotels/:id/permit-url", async (req, res) => {
    const id = req.params.id;
    const { data: hotel, error: hotelError } = await supabaseClient_1.supabaseAdmin
        .from("hotels")
    ...
    const { data: signed, error: signError } = await supabaseClient_1.supabaseAdmin.storage
        .from("hotel-assets")
...
router.patch("/admin/hotels/:id/verify", async (req, res) => {
    ...
    let result = await supabaseClient_1.supabaseAdmin
        .from("hotels")
        .update(updates)
        .eq("id", id)
        .select("*")
    ...
    if (result.error && (result.error.message?.includes("permit_expires_at") ?? result.error.message?.includes("schema cache"))) {
        result = await supabaseClient_1.supabaseAdmin
            .from("hotels")
...
router.delete("/admin/users/:id", async (req, res) => {
    ...
    const { error: reviewsErr } = await supabaseClient_1.supabaseAdmin
        .from("reviews")
        .delete()
        .eq("user_id", id);
    if (reviewsErr) {
    ...
    const { error: bookingsErr } = await supabaseClient_1.supabaseAdmin
        .from("bookings")
        .delete()
        .eq("user_id", id);
    if (bookingsErr) {
    ...
    const { error: prefsErr } = await supabaseClient_1.supabaseAdmin
    ...
    const { error: profileErr } = await supabaseClient_1.supabaseAdmin
        .from("profiles")
        .delete()
        .eq("id", id);
    if (profileErr) {
